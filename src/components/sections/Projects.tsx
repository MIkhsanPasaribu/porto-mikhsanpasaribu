...
export default function ProjectsSection() {
    ...
    if (
      ...
      techList.some(
        (tech) =>
          ...
          (techList.some((t) => t.includes("frontend")) &&
            techList.some((t) => t.includes("backend")))
      )
    ) {
      return "Fullstack";
    } else if (
  ...
  const handleImageError = (projectId: number, projectTitle: string) => {
    console.error(`Failed to load image for project: ${projectTitle}`);
    ...
    return (
      <section id="projects" className="py-16 md:py-24">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <span className="font-jetbrains-mono">{""}</span> Projects{" "}
            <span className="font-jetbrains-mono">{""}</span>
          </h2>
            ...
            <div className="flex space-x-6 px-4 w-max">
              {projects.map((project) => (
                ...
                >
                  {" "}
                  {/* Project Image */}
                  {project.image_url && !failedImages.has(project.id) ? (
                    <div className="relative w-full h-[180px]">
                      <Image
                        ...
                        onError={() =>
                          handleImageError(project.id, project.title)
                        }
                      />
                      <div
                        className={`absolute top-3 left-3 px-3 py-1 text-xs rounded-full ${
                          isDarkMode
                            ? "bg-[#0A0A0A]/90 text-[#19A7CE]"
                            : "bg-white/90 text-[#0B409C]"
                        }`}
                      >
